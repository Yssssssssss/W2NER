...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in enumerate(tqdm(data_loader,desc="train")):
            ...
            self.scheduler.step()
        # todo 观察是不是这一步导致内存激增
        print("*"*20+"cat-start"+"*"*20)
        print("label_result=",len(label_result))
        label_result = torch.cat(label_result)
        print("label_result",label_result.shape)
        print("pred_result=", len(label_result))
        pred_result = torch.cat(pred_result)
        print("pred_result", label_result.shape)
        print("*" * 20 + "cat-end" + "*" * 20)
        print("*" * 20 + "precision_recall_fscore_support-start" + "*" * 20)
        p, r, f1, _ = precision_recall_fscore_support(label_result.numpy(),
                                                      pred_result.numpy(),
                                                      average="macro")
        print("*" * 20 + "precision_recall_fscore_support-end" + "*" * 20)
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        total_ent_c = 0
        with torch.no_grad():
            for i, data_batch in enumerate(tqdm(data_loader,desc="eval")):
